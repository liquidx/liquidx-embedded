...
class HalGPIO {
 ...
 public:
  ...
  // Background button sampling (InputManager::beginAsync): a task polls every
  // pollMs and queues press edges so presses during a blocking e-paper refresh
  // are not lost. Once started, do not call update()/wasPressed(); drain with
  // popPress(). isPressed()/getPowerButtonHeldTime() stay valid (task-updated).
  void beginAsyncInput(uint32_t pollMs = 15) { inputMgr.beginAsync(2, pollMs); }
  bool popPress(uint8_t& button) { return inputMgr.popPress(button); }
